...
const EditModal = ({ open, handleOpen, data }) => {
  const [formData, setFormData] = useState({ id: "", location: "", fill: "" });
  ...
  useEffect(() => {
    if (data) {
      setFormData({ id: data.id, location: data.location, fill: data.fill });
      setStartDate(null);
      setEndDate(null);
    }
  }, [data]);
  ...
  return (
    <Dialog open={open} handler={handleOpen} size="sm" className="rounded-[28px] overflow-visible shadow-2xl border border-blue-50/50">
      ...
      <DialogBody className="px-8 py-6 space-y-7">
        ...
        <div className="space-y-4">
          <div className="relative">
            <Typography className="text-[11px] font-black text-[#2b6cb0] uppercase ml-1 mb-2 tracking-widest opacity-70">Identitas Kendaraan</Typography>
            <Input 
              disabled 
              value={formData.id} 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
          <div className="space-y-2">
            <Typography className="text-[11px] font-black text-[#2b6cb0] uppercase ml-1 tracking-widest opacity-70">Update Status</Typography>
            <Input 
              type="number" 
              value={formData.fill}
          ...
          <div className="space-y-2">
            <Typography className="text-[11px] font-black text-[#2b6cb0] uppercase ml-1 tracking-widest opacity-70">Jadwal Servis</Typography>
            <button 
              type="button"
            ...
            >
              <div className="flex items-center gap-2 truncate text-xs font-semibold">
                ...
                <span className={startDate ? 'text-blue-900' : 'text-gray-400'}>
                  {startDate ? (endDate ? `${formatDate(startDate)} - ${formatDate(endDate)}` : formatDate(startDate)) : "Set Ulang Jadwal"}
                </span>
              </div>
      ...
      <DialogFooter className="flex items-center justify-end gap-3 px-8 pb-8 pt-2">
        <Button variant="text" color="blue-gray" onClick={handleOpen} className="normal-case font-bold px-6 py-3 rounded-xl hover:bg-gray-100">Batal</Button>
        <Button className="bg-[#66bb6a] px-8 py-3 rounded-xl normal-case font-bold shadow-none active:scale-95 transition-all" onClick={handleOpen}>
          Simpan Perubahan
        </Button>
      </DialogFooter>
    </Dialog>
