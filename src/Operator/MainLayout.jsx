...
const MainLayout = ({ children }) => {
  ...
  // 1. Logika Deteksi Role berdasarkan URL (Ditambah Admin Store)
  const isAreaAdmin = location.pathname.startsWith("/area");
  const isOperator = location.pathname.startsWith("/operator");
  const isStoreAdmin = location.pathname.startsWith("/store"); // Deteksi Admin Store

  const handleLogout = () => {
    navigate("/"); 
  };

  // Penentuan Label Role dan Warna Dinamis
  const getRoleLabel = () => {
  ...
  const getThemeColor = () => {
    ...
    if (isStoreAdmin) return "text-green-600"; // Warna Hijau untuk Store
  ...
  const getAvatarBg = () => {
    ...
    if (isStoreAdmin) return "bg-green-600 shadow-green-100"; // BG Hijau untuk Store
  ...
  return (
    <div className="flex h-screen w-full bg-[#f8fafc] overflow-hidden">
      <Sidebar 
        ...
        isStore={isStoreAdmin} // Kirim props isStore ke Sidebar
      />
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block w-48 md:w-80">
              <Input
                ...
                className={`!border-t-blue-gray-100 ${isStoreAdmin ? "focus:!border-green-500" : "focus:!border-orange-500"} bg-gray-50/50 rounded-2xl`}
