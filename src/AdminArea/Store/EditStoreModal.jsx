      ...
      <DialogHeader className="flex justify-between px-8 pt-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <Typography className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">ID Toko: #{data?.id}</Typography>
          </div>
        </div>
        <XMarkIcon className="h-5 w-5 cursor-pointer text-gray-400 hover:text-red-500 transition-colors" onClick={handleOpen} />
      </DialogHeader>
      ...
      <DialogBody className="px-8 py-4 space-y-6 overflow-y-auto max-h-[70vh]">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center border-b pb-1">
             <Typography className="font-bold text-blue-600 text-xs uppercase tracking-wider">Informasi Toko</Typography>
             <Typography className="text-[10px] bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full font-bold">Area: {areaName}</Typography>
          </div>
          
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <Typography variant="small" className="text-blue-900 font-bold ml-1">Nama Toko</Typography>
              <Input name="name" value={formData.name} onChange={handleChange} labelProps={{ className: "hidden" }} className="!rounded-xl !border-blue-gray-100 focus:!border-blue-500 bg-white" />
            </div>
            <div className="space-y-1">
              <Typography variant="small" className="text-blue-900 font-bold ml-1">Wilayah Kerja</Typography>
              <Input value={areaName} disabled labelProps={{ className: "hidden" }} className="!rounded-xl bg-gray-50 border-none font-medium" />
            </div>
          </div>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="bg-orange-50 p-3 rounded-xl">
             <Typography className="text-[10px] text-orange-800 italic leading-tight">
              * Perubahan akun di sini akan langsung memperbarui kredensial login admin toko tersebut. Pastikan data sudah benar.
            </Typography>
          </div>
        </div>
